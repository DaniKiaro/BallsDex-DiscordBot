    ...
    async def claim(self, interaction: discord.Interaction, button: ui.Button):
        if self.claimed:
            await interaction.response.send_message(
                "This Dex has already been claimed.", ephemeral=True
            )
            return
...
class Balls(app_commands.Group):
    ...
    @app_commands.command(name="gdrop", description="Drop any Dexer.")
    @app_commands.checks.has_any_role(*settings.root_role_ids)
    async def gdrop(self, interaction: Interaction, Dexer: BallEnabledTransform):
        ...
        ball_instance = await BallInstance.create(ball=Dex, player=player)

        view = GDropView(ball_instance)  # 👈 Pass BallInstance
    
        embed = Embed(
                title=f"{Dex} has been dropped!",
                description=(
                    f"⚽ Dropped by {interaction.user.mention}\n\n"
                    f"Click the button below to claim **{Dex}**!\n"
                    f"Rarity: **{Dex.rarity}**"
                ),
                color=0x2ECC71,
            )
